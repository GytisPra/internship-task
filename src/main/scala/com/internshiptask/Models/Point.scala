...
object Point {
  ...
  def apply(long: Double, lat: Double): Either[String, Point] =
    if long > 180 || long < -180 then
      Left(s"provided longitude ($long) can't be more than 180 or less than -180 degrees")
    else if lat > 90 || lat < -90 then
      Left(s"provided latitude ($lat) can't be more than 90 or less than -90 degrees")
